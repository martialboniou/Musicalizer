...
static void *assoc_find_(void *items, size_t item_size, size_t items_count,
                         size_t item_value_offset, const char *key)
{
...
#define assoc_find(table, key)                                                 \
    assoc_find_((table).items, sizeof((table).items[0]), (table).count,        \
...
static void assets_unload_everything()
{
...
#ifdef FEATURE_MICROPHONE
static void ma_callback(ma_device *pDevice, void *pOutput, const void *pInput,
...
static void error_load_file_popup()
{
...
static void volume_slider(Rectangle preview_boundary)
{
    ...
    float icon_size = 512;
    float scale = HUD_BUTTON_SIZE / icon_size * HUD_ICON_SCALE;
    Rectangle dest = {volume_slider_boundary.x + (float)HUD_BUTTON_SIZE / 2 -
                          icon_size * scale / 2,
                      volume_slider_boundary.y + (float)HUD_BUTTON_SIZE / 2 -
                          icon_size * scale / 2,
                      icon_size * scale, icon_size * scale};
...
static void preview_screen()
{
    ...
    if (IsKeyPressed(KEY_M)) {
        ...
        if (p->microphone != NULL) {
            ...
            if (result != MA_SUCCESS) {
                TraceLog(LOG_ERROR, "MINIAUDIO: Failed to start device: %s",
                         ma_result_description(result));
    ...
    if (track) { // music is loaded and ready
        ...
        if (IsKeyPressed(KEY_R)) {
            ...
            p->ffmpeg = ffmpeg_start_rendering(p->screen.texture.width,
        ...
        // TODO: add tooltips to all the buttons that describe their
        // function and associated keyboard shortcuts
        ...
        if (p->fullscreen) {
            ...
            if (hud_timer > 0.0) {
                ...
                // TODO: the state of volume slider does not reset
                // hud_timer
