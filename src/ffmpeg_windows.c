...
// NOTE: We can't include raylib.h in here cause it collides with windows.h. So
// we just copy pasted log related declarations.
...
FFMPEG *ffmpeg_start_rendering(size_t width, size_t height, size_t fps,
    ...
    if (!CreatePipe(&pipe_read, &pipe_write, &saAttr, 0)) {
        TraceLog(LOG_ERROR,
                 "FFMPEG: Could not create pipe. System Error Code: %d",
                 GetLastError());
        return NULL;
    }
    ...
    if (!SetHandleInformation(pipe_write, HANDLE_FLAG_INHERIT, 0)) {
        TraceLog(LOG_ERROR,
                 "FFMPEG: Could not mark write pipe as non-inheritable. System "
                 "Error Code: %d",
                 GetLastError());
        return NULL;
    }
    ...
    if (siStartInfo.hStdError == INVALID_HANDLE_VALUE) {
        TraceLog(LOG_ERROR,
                 "FFMPEG: Could get standard error handle for the child. "
                 "System Error Code: %d",
                 GetLastError());
        return NULL;
    }
    ...
    if (siStartInfo.hStdOutput == INVALID_HANDLE_VALUE) {
        TraceLog(LOG_ERROR,
                 "FFMPEG: Could get standard output handle for the child. "
                 "System Error Code: %d",
                 GetLastError());
        return NULL;
    }
    ...
    snprintf(cmd_buffer, sizeof(cmd_buffer),
             "ffmpeg.exe -loglevel verbose -y -f rawvideo -pix_fmt rgba -s "
             "%dx%d -r %d -i - -i \"%s\" -c:v libx264 -vb 2500k -c:a aac -ab "
             "200k -pix_fmt yuv420p output.mp4",
             (int)width, (int)height, (int)fps, sound_file_path);

    if (!CreateProcess(NULL, cmd_buffer, NULL, NULL, TRUE, 0, NULL, NULL,
                       &siStartInfo, &piProcInfo)) {
        TraceLog(
            LOG_ERROR,
            "FFMPEG: Could not create child process. System Error Code: %d",
            GetLastError());
...
bool ffmpeg_send_frame_flipped(FFMPEG *ffmpeg, void *data, size_t width,
    ...
    for (size_t y = height; y > 0; --y) {
        ...
        if (!WriteFile(ffmpeg->hPipeWrite, (uint32_t *)data + (y - 1) * width,
                       sizeof(uint32_t) * width, NULL, NULL)) {
            TraceLog(LOG_ERROR,
                     "FFMPEG: failed to write into ffmpeg pipe. System Error "
                     "Code: %d",
                     GetLastError());
            return false;
        }
...
bool ffmpeg_end_rendering(FFMPEG *ffmpeg) {
    ...
    if (WaitForSingleObject(hProcess, INFINITE) == WAIT_FAILED) {
        TraceLog(
            LOG_ERROR,
            "FFMPEG: could not wait on child process. System Error Code: %d",
            GetLastError());
    ...
    if (GetExitCodeProcess(hProcess, &exit_status) == 0) {
        TraceLog(
            LOG_ERROR,
            "FFMPEG: could not get process exit code. System Error Code: %d",
            GetLastError());
        CloseHandle(hProcess);
        return false;
    }
    ...
    if (exit_status != 0) {
        TraceLog(LOG_ERROR, "FFMPEG: command exited with exit code %lu",
                 exit_status);
        CloseHandle(hProcess);
        return false;
    }
