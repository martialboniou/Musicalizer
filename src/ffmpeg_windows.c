...
FFMPEG *ffmpeg_start_rendering(size_t width, size_t height, size_t fps, const char *audio_file_path)
{
    HANDLE pipe_read;
    HANDLE pipe_write;
    ...
    snprintf(cmd_buffer, sizeof(cmd_buffer), "ffmpeg.exe -loglevel verbose -y -f rawvideo -pix_fmt rgba -s %dx%d -r %d -i - -i %s -c:v libx264 -vb 2500k -c:a aac -ab 200k -pix_fmt yuv420p output.mp4", width, height, fps, audio_file_path);
